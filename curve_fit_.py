...
#-----------------------------------------------------------------------------------------------------------------
def objective_function_recoverd_dead(t,  contacts,X):
    #initial_conditions = [S0, E0, A0, I0, F0, R0, D0]
    #contacts = 3.0
    ...
    recovered = temp.y[5]#np.where(X[:,1],y1,y2)
    dead = temp.y[6]
    daily_recovered_ = []
    daily_death_ = []
...
print(f'length of the time t_fit_{len(t_fit)}, tmax{(tmax)}, time t {len(t)},time t_fit_{len(t_fit)}, recovered_dead{len(recovered_dead)}')
